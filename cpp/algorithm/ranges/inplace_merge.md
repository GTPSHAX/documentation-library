...
Exactly `N − 1` comparisons, if additional memory buffer is available, where `1=N = ranges::distance(first, last)`. Otherwise, mathjax-or|\(\scriptsize \mathcal{O}(N\cdot\log{(N)})\)|𝓞(N•log(N)) comparisons. Additionally, twice as many projections as comparisons in both cases.

...
On a range that's fully sorted (or more generally, partially ordered with respect to `value`) after projection, `std::ranges::lower_bound` implements the binary search algorithm. Therefore, `ranges::binary_search|std::ranges::binary_search` can be implemented in terms of it.

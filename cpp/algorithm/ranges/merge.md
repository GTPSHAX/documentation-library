...
A sequence is said to be ''sorted'' with respect to the comparator `comp` if for any iterator `it` pointing to the sequence and any non-negative integer `n` such that `it + n` is a valid iterator pointing to an element of the sequence, `1=std::invoke(comp, std::invoke(proj2, *(it + n)), std::invoke(proj1, *it)))` evaluates to `false`.
...
`1= N = ranges::distance(first1, last1) + ranges::distance(first2, last12)`.

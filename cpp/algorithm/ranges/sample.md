...
1. Selects `1= M = min(n, last - first)` elements from the sequence [first, last) (without replacement) such that each possible ''sample'' has equal probability of appearance, and writes those selected elements into the range beginning at `out`.
...
- `[3=from which to make the sampling (''the population''), sentinel=yes}})` - 

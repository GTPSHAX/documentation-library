...
* has no effects, if `1=order == memory_order_relaxed`
* is an acquire fence, if `1=order == memory_order_acquire
* is a release fence, if `1=order == memory_order_release`
* is both an acquire fence and a release fence, if `1=order == memory_order_acq_rel`
* is a sequentially consistent acquire and release fence, if `1=order == memory_order_seq_cst`

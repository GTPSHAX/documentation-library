---
title: operators (std::chrono::duration)
type: Utilities
source: https://en.cppreference.com/w/cpp/chrono/duration/operator_arith4
---


# 1=operator+,-,*,/,%small|(std::chrono::duration)
...
.
5. Converts the duration `d` to one whose `rep` is the common type between `Rep1` and `Rep2`, and divides the number of ticks after conversion by `s`. .
...
7. Converts the duration `d` to one whose `rep` is the common type between `Rep1` and `Rep2`, and creates a duration whose tick count is the remainder of the division of the tick count, after conversion, by `s`. .
...
Assuming that `CD` is the function return type and `CD<A, B> , then:

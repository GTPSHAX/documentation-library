...
Converts the `std::chrono::local_time|local_time` `tp` in this time zone to the corresponding `std::chrono::sys_time|sys_time`.
...
*If `1=z == std::chrono::choose::earliest`, returns the earlier `sys_time`.
*If `1=z == std::chrono::choose::latest`, returns the later `sys_time`.

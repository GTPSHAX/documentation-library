...
1. Constructs a `std::chrono::weekday_indexed|weekday_indexed` from `*this` and `index`. The result represents the `index`-th weekday in some yet-to-be-specified month. If `index` is not in the range  or if `!ok()` the values (an underlying weekday and an index) held in the result are unspecified.
2. Constructs a `std::chrono::weekday_last|weekday_last` from `*this`. The result represents the last weekday in some yet-to-be-specified month.

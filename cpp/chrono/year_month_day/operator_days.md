---
title: std::chrono::year_month_day::operators
type: Utilities
...
@@ A `std::chrono::sys_days` in the range c|[std::chrono::days{-12687428}, std::chrono::days{11248737}], when converted to `year_month_day` and back, yields the same value.
2. Same as  but returns `std::chrono::local_days|local_days` instead. Equivalent to `return local_days(sys_days(*this).time_since_epoch());`.

...
# deduction guides for tt|std::span
...
1. Allows the element type to be deduced from the iterator-sentinel pair. <sup>(since C++26)</sup> It also allows the static extent to be deduced if `EndOrSize` satisfies . .
@2-4@ Allows the static extent to be deduced from built-in arrays and `std::array`.
5. Allows the element type to be deduced from ranges. .

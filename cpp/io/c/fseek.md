...
POSIX also requires that `fseek` first performs `std::fflush|fflush` if there are any unwritten data (but whether the shift state is restored is implementation-defined). The standard C++ file streams guarantee both flushing and unshifting: `std::basic_filebuf::seekoff`.
POSIX specifies, that [https://pubs.opengroup.org/onlinepubs/9699919799/functions/fseek.html `fseek`] should return `-1` on error, and set `errno` to indicate the error.

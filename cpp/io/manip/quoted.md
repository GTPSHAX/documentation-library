...
Allows insertion and extraction of quoted strings, such as the ones found in [Comma-separated values|CSV](https://en.wikipedia.org/wiki/Comma-separated values|CSV) or [XML](https://en.wikipedia.org/wiki/XML).
...
:Finally, outputs each character from the resulting sequence as if by calling `out.rdbuf()->sputn(seq, n)`, where `1=n=std::max(out.width(), seq.size())` and `out.width(0)` to cancel the effects of `std::setw`, if any.
...
::@3@ Extracts successive characters from `in` and appends them to `s`, except that whenever an `escape` character is extracted, it is ignored and the next character is appended to `s`. Extraction stops when `1= !in == true` or when an unescaped `delim` character is found.

...
# Class template argument deduction (CTAD) mark since c++17
...
* If `C` is defined, for each constructor (or constructor template) `C declared in the named primary template, a fictional function template `F, is constructed, such that all following conditions are satisfied:
:* The template parameters of `F are the template parameters of `C` followed (if `C is a constructor template) by the template parameters of `C (default template arguments are included too).
rrev|since=c++20|
:* The `associated constraints` of `F are the conjunction of the associated constraints of `C` and the associated constraints of `C.
:* The  of `F is the parameter list of `C.
:* The return type of `F is `C` followed by the template parameters of the class template enclosed in `<>`.
...
* For each user-defined deduction guide `G, a fictional function or function template `F, is constructed, such that all following conditions are satisfied:
:* The parameter list of `F is the parameter list of `G.
:* The return type of `F is the simple template identifier of `G.
:* If `G has template parameters (syntax ), `F is a function template, and its template parameter list is the template parameter list of `G. Otherwise, `F is a function.
rrev|since=c++20|1=
* In addition, if
...
:* the variable is initialized from a non-empty list of initializers *arg1, arg2, ..., argn* (which may use ),
...
:* Let `e be the (possibly recursive) `aggregate element` that would be initialized from `arg, where
::*  is not considered for any aggregate element that has
:::* a `dependent` non-array type,
:::* an array type with a `value-dependent` bound, or
:::* an array type with a dependent array element type and `arg is a `string literal`
...
:* If there is no such `e, the aggregate deduction candidate is not added.
:* Otherwise, determine the parameter list `T of the aggregate deduction candidate as follows:
::* If `e is an array and `arg is a braced-init-list, `T is an rvalue reference to the declared type of `e.
::* If `e is an array and `arg is a `string literal`, `T is an lvalue reference to the const-qualified declared type of `e.
::* Otherwise, `T is the declared type of `e.
::* If a pack was skipped because it is a non-trailing aggregate element, an additional parameter pack of the form `P is inserted in its original aggregate element position. (This will generally cause deduction to fail.)
::* If a pack is a trailing aggregate element, the trailing sequence of parameters corresponding to it is replaced by a single parameter of the form `T.
: The aggregate deduction candidate is a fictional function template derived as above from a hypothetical constructor `C(T.
...
- `*template-name* **`(`** *parameter-list* **`)`** **`->`** *simple-template-id* *requires-clause* (optional) **`;`**`
- `template-parameter-list**`>`** *requires-clause* (optional)<br>*explicit* (optional) *template-name* **`(`** *parameter-list* **`)`** **`->`** *simple-template-id* *requires-clause* (optional) **`;`**`

### Parameters

- `{{spar` - template-parameter-list|a non-empty comma-separated list of `template parameters`
- `{{spar` - explicit|an ``explicit` specifier`
- `{{spar` - template-name|the name of the class template whose arguments are to be deduced
- `{{spar` - parameter-list|a (possibly empty) 
- `{{spar` - simple-template-id|a `simple template identifier`
- `{{spar` - requires-clause|<sup>(C++20)</sup> a ``requires` clause`
rrev|since=c++20|
The parameters of user-defined deduction guides cannot have placeholder types: the  syntax is not allowed.
...
| WG | Std | Before | After |
|----|------|--------|-------|
| cwg-2376 | C++17 | CTAD would be performed even if the type of the variable declared is<br>different from the class template whose arguments will be deduced | do not perform<br>CTAD in this case |
| cwg-2628 | C++20 | implicit deduction guides did not propagate constraints | propogate constraints |
| cwg-2697 | C++20 | it was unclear whether the abbreviated function template<br>syntax is allowed in user-defined deduction guides | prohibited |
| cwg-2714 | C++17 | implicit deduction guides did not consider<br>the default aguments of constructors | consider them |

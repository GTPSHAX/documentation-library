...
The `const` qualifier used on a declaration of a non-local non-volatile <sup>(since C++14)</sup> non-`template`<sup>(since C++17)</sup> non-`inline` variable that is not declared `extern` gives it `internal linkage`. This is different from C where const file scope variables have external linkage.
...
| cwg-1528 | C++98 | there was no requirement on the number of occurrences<br>of each cv-qualifier in the same cv-qualifier sequence | at most once for<br>each cv-qualifier |

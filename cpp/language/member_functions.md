...
`Constructors`, `destructor`s, and `conversion functions` use special syntaxes for their declarations. The rules described in this page may not apply to these functions. See their respective pages for details.
rrev|since=c++23|
...
Any `function declarations` are allowed, with additional syntax elements that are only available for non-static member functions: `pure-specifiers`, cv-qualifiers<sup>(since C++11)</sup> , ref-qualifiers, `override` specifiers, and `member initialization lists`.
...
For a non-static non-virtual member function not declared with cv-qualifier or ref-qualifier, its first parameter, if not being a `function parameter pack`, can be an  (denoted with the prefixed keyword `this`):
...
This makes it possible to deduplicate const- and non-const member functions, see  for an example.
...
* <sup>(until C++20)</sup> `Destructor`<sup>(since C++20)</sup> `Prospective destructor`
Special member functions <sup>(since C++20)</sup> along with the `comparison operators` are the only functions that can be ''defaulted'', that is, defined using `1== default` instead of the function body (see their pages for details).
...
| WG | Std | Before | After |
|----|------|--------|-------|
| cwg-194 | C++98 | ambiguous whether a non-static member function<br>could have the same name as the enclosing class name | explicit naming restriction added |

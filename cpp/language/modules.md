...
# Modules mark since c++20

Most C++ projects use multiple translation units, and so they need to share `declarations` and `definition`s across those units. The usage of  is prominent for this purpose, an example being the  whose declarations can be provided by including the corresponding header.
...
Modules are orthogonal to `namespace`s.
...
- `sdsc|num=1|`
- `**`export`** **`module`** *module-name module-partition* (optional) *attr* (optional) **`;`**`
- `sdsc|num=2|`
- `**`export`** *declaration*`
- `sdsc|num=3|`
- `**`export {`** *declaration-seq* (optional) }`
- `sdsc|num=4|`
- `**`export`** **`import`** *module-name attr* (optional) **`;`**`
- `sdsc|num=5|`
- `**`export`** **`import`** *module-partition attr* (optional) **`;`**`
- `sdsc|num=6|`
- `**`export`** **`import`** *header-name attr* (optional) **`;`**`
- `sdsc|num=7|`
- `**`module;`**`
- `sdsc|num=8|`
- `**`module : private;`**`
...
`cpp/preprocessor/include|#include` should not be used in a module unit (outside the ''global module fragment''), because all included declarations and definitions would be considered part of the module. Instead, headers can also be imported as ''header units'' with an ''import declaration'':
...
`cpp/keyword/private`,
`cpp/identifier_with_special_meaning/module`,
`cpp/identifier_with_special_meaning/import`,
`cpp/keyword/export`
...
| WG | Std | Before | After |
|----|------|--------|-------|
| cwg-2732 | C++20 | it was unclear whether importable headers can<br>react to preprocessor state from the point of import | no reaction |

...
::* `destructor`s unless the destructor of any potentially-constructed base or member is ''potentially-throwing'' (see below)
::* `default constructor`s, `copy constructor`s, `move constructor`s that are implicitly-declared or defaulted on their first declaration unless
...
Functions differing only in their exception specification cannot be overloaded <sup>(since C++17)</sup> (just like the return type, exception specification is part of function type, but not part of the function signature).
...
Pointers (including pointers to member function) to non-throwing functions <sup>(until C++17)</sup> can be assigned to or used to initialize<sup>(since C++17)</sup> are `implicitly convertible to` pointers to potentially-throwing functions, but not the other way around.
...
* `e` is a function call to a function, pointer to function, or pointer to member function which is ''potentially-throwing''<sup>(until C++17)</sup> , unless `e` is a `core constant expression`
...
`cpp/keyword/noexcept`rev inl|since=c++17|until=c++20|,
`cpp/keyword/throw`
...
| cpp/language/dsc noexcept | (see dedicated page) |
| cpp/language/dsc except spec | (see dedicated page) |
| cpp/language/dsc throw | (see dedicated page) |

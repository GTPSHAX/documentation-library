...
- `{{spar` - typedef-name|an identifier or a simple-template-id that names a pack
- `{{spar` - id-expression|an id-expression that names a pack
- `{{spar` - expression|a `converted constant expression` `I` of type `std::size_t` designated as index where `I` is within the range  for some pack `P` in pack indexing
...
Let `P` be a non-empty pack containing `P and `I` be a valid index, the instantiation of the expansion `P...[I]` yields the pack element `P of `P`.
...
- `**`...[`** *expression* **`]`**`
Pack indexing expression denotes the ''id-expression'', the expression of pack element `P. The *id-expression* shall be introduced by the declaration of:
...
- `**`...[`** *expression* **`]`**`
Pack indexing specifier denotes the ''computed-type-specifier'', the type of pack element `P. The *typedef-name* shall be introduced by the declaration of `type template parameter pack`.
...
Pack indexing specifier can be used in function or constructor parameter list to establish  in template argument deduction.

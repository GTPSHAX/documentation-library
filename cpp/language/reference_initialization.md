...
#### Ordinary list-initialization <sup>(C++11)</sup>


**Syntax:**

- `sdsc|num=1|`
- `T**`&`** *ref* **`= {`** *arg1***`,`** *arg2***`,`** ... ttb|};<br>`
- `T**`&`** *ref* **`{`** *arg1***`,`** *arg2***`,`** ... ttb|};`
- `sdsc|num=2|`
- `T**`&&`** *ref* **`= {`** *arg1***`,`** *arg2***`,`** ... ttb|};<br>`
- `T**`&&`** *ref* **`{`** *arg1***`,`** *arg2***`,`** ... ttb|};`
- `sdsc|num=3|`
- `*func-refpar* **`({`** *arg1***`,`** *arg2***`,`** ... ttb|});`

#### Designated list-initialization <sup>(C++20)</sup>


**Syntax:**

- `sdsc|num=1|`
- `T**`&`** *ref* **`= {.`***des1* **`=`** *arg1* **`, .`***des2* **`{`** *arg2* } ... ttb|};<br>`
- `T**`&`** *ref* **`{.`***des1* **`=`** *arg1* **`, .`***des2* **`{`** *arg2* } ... ttb|};`
- `sdsc|num=2|`
- `T**`&&`** *ref* **`= {.`***des1* **`=`** *arg1* **`, .`***des2* **`{`** *arg2* } ... ttb|};<br>`
- `T**`&&`** *ref* **`{.`***des1* **`=`** *arg1* **`, .`***des2* **`{`** *arg2* } ... ttb|};`
- `sdsc|num=3|`
- `*func-refpar* **`({.`***des1* **`=`** *arg1* **`, .`***des2* **`{`** *arg2* } ... ttb|});`
...
4. In the `return` statement, when the function returns a reference type. <sup>(since C++26)</sup> The program is ill-formed if the returned reference is bound to the result of a temporary expression.
...
- `{{spar` - T|the referenced type
- `{{spar` - ref|the reference variable to be initialized
- `{{spar` - target|the initializer expression being used
- `{{spar` - func-refpar|a function with a parameter of reference type (T`&`<sup>(since C++11)</sup>  or T`&&`)
- `{{spar` - func-refret|a function whose returns type is a reference type (T`&`<sup>(since C++11)</sup>  or T`&&`)
- `{{spar` - Class|a class name
- `{{spar` - ref-member|a non-static data member of reference type (T`&`<sup>(since C++11)</sup>  or T`&&`) of *Class*
- `{{spar` - des1, *des2*, ...|designators
- `{{spar` - arg1, *arg2*, ...|the initializers in initializer lists
...
rrev|since=c++11|
If a reference initialization uses an ordinary<sup>(since C++20)</sup>  or designated list-initialization, the rules of `list-initialization` are followed.
...
rrev|since=c++17|
If *target* is a prvalue,  is applied to it, considering the type of the prvalue to be the adjusted type `P`.
...
rrev|since=c++17|
If the result of the conversion is a prvalue,  is applied to it, considering the type of the prvalue to be the adjusted type `P`.
...
rrev multi|rev1=
* a  expression of an object type,
|since2=c++17|rev2=
...
Until the resolution of , a temporary is permitted to bound to a reference member in a constructor `initializer list`, and it persists only until the constructor exits, not as long as the object exists. Such initialization is ill-formed since , although many compilers still support it (a notable exception is clang).
...
| WG | Std | Before | After |
|----|------|--------|-------|
| cwg-391 | C++98 | initialize a reference to const-qualified type with a class type<br>rvalue might create a temporary, and a constructor of that class<br>was required in order to copy the rvalue into that temporary | no temporary is<br>created, constructor<br>is not required |
| cwg-450 | C++98 | a reference to const-qualified array could not be<br>initialized with a reference-compatible array rvalue | allowed |
| cwg-589 | C++98 | a reference could not bind directly to an array or class rvalue | allowed |
| cwg-656 | C++98 | a reference to const-qualified type initialized with a type which is not<br>reference-compatible but has a conversion function to a reference-<br>compatible type was bound to a temporary copied from the return<br>value (or its base class subobject) of the conversion function | bound to the return<br>value (or its base class<br>subobject) directly |
...
| cwg-2481 | C++17 | cv-qualification was not added to the result type<br>of temporary materialization in indirect binding | added |
| cwg-2657 | C++17 | cv-qualification was not added to the result type<br>of temporary materialization in direct binding | added |

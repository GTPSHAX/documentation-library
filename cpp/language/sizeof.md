...
- `{{spar` - type|a *type-id* (see `type naming`)
- `{{spar` - expression|an expression whose `operator precedence` is not lower than `sizeof` (e.g. `sizeof a + b` is parsed as `(sizeof a) + b` instead of `sizeof (a + b)`)
The result of a `sizeof` expression is a `constant expression` of type `std::size_t`.

## Notes

Depending on the computer architecture, a [byte](https://en.wikipedia.org/wiki/byte) may consist of 8 or more bits, the exact number being recorded in `CHAR_BIT`.
...
rev|since=c++20|
* `sizeof(char8_t)`
`sizeof` cannot be used with function types, incomplete types, or bit-field <sup>(until C++11)</sup> lvalues<sup>(since C++11)</sup> glvalues.
...
When applied to an expression, `sizeof` does `not evaluate the expression`<sup>(since C++11)</sup>  (i.e. the expression is an unevaluated operand), and even if the expression designates a polymorphic object, the result is the size of the static type of the expression. Lvalue-to-rvalue, array-to-pointer, or function-to-pointer conversions are not performed.<sup>(since C++17)</sup>  `Temporary materialization, however, is (formally) performed for prvalue arguments: the program is ill-formed if the argument is not destructible.`
...
| cpp/language/dsc sizeof... | (see dedicated page) |

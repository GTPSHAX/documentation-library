...
In order to instantiate a `function template`, every template argument must be known, but not every template argument has to be specified. When possible, the compiler will deduce the missing template arguments from the function arguments. This occurs when a function call is attempted, when an address of a function template is taken, and in some :
...
Template argument deduction takes place after the function template `name lookup` (which may involve `argument-dependent lookup`) and before  (which may involve `SFINAE`) and `overload resolution`.
rrev|since=c++17|
...
Template argument deduction attempts to determine template arguments (types for type template parameters **`T`**i, templates for template template parameters **`TT`**i, and values for non-type template parameters **`I`**i), which can be substituted into each function parameter type **`P`** to produce the type ''deduced'' **`A`**, which is the same as the type of the argument **`A`**, after adjustments listed below.
...
4. If **`P`** is an rvalue reference to a cv-unqualified template parameter (so-called ), and the corresponding function call argument is an lvalue, the type lvalue reference to **`A`** is used in place of **`A`** for deduction (Note: this is the basis for the action of `std::forward`<sup>(since C++17)</sup> . Note: in `class template argument deduction, template parameter of a class template is never a forwarding reference`):
...
After these transformations, the deduction processes as described below (cf. section ) and attempts to find such template arguments that would make the deduced **`A`** (that is, **`P`** after adjustments listed above and the substitution of the deduced template parameters) identical to the ''transformed'' **`A`**, that is **`A`** after the adjustments listed above.
...
2. The transformed **`A`** can be another pointer or pointer to member type that can be converted to the deduced **`A`** via a <sup>(since C++17)</sup>  or a function pointer conversion:
...
3. If **`P`** is a class and **`P`** has the form `simple-template-id` (template name with a template argument list)<sup>(since C++26)</sup>  or `splice specifier with a template argument list`, then the transformed **`A`** can be a derived class of the deduced **`A`**. Likewise, if **`P`** is a pointer to a class of the form ''simple-template-id'' (template name with a template argument list)<sup>(since C++26)</sup>  or `splice specifier with a template argument list`, the transformed **`A`** can be a pointer to a derived class pointed to by the deduced **`A`**:
...
rev|since=c++26|
4. The expression of a `splice specifier`:

```cpp
template<std::meta::info R>
void f(typename[:R:] arg);

int n;
f<^^int>(n); // P = typename[:R:], A = ^^int: R is in non-deduced context
```

5. A non-type template argument or an array bound in which a subexpression references a template parameter:
...
6. A template parameter used in the parameter type of a function parameter that has a default argument that is being used in the call for which argument deduction is being done:
...
7. The parameter **`P`**, whose **`A`** is a function or a set of overloads such that more than one function matches **`P`** or no function matches **`P`** or the set of overloads includes one or more function templates:
...
8. The parameter **`P`**, whose **`A`** is a braced-init-list, but **`P`** is not `std::initializer_list`, a reference to one (possibly cv-qualified), or a reference to an array:
...
rrev|since=c++11|1=
9. The parameter **`P`** which is a parameter pack and does not occur at the end of the parameter list:
...
10. The template parameter list that appears within the parameter **`P`**, and which includes a pack expansion that is not at the very end of the template parameter list:
...
11. For **`P`** of array type (but not reference to array or pointer to array), the major array bound:
...
* `TT``&lt;>`.
In the above forms,
* `T` or `U` represents a type or *parameter-type-list* that either satisfies these rules recursively, is a non-deduced context in `P` or `A`, or is the same non-dependent type in `P` and `A`.
* `TT` or `TU` represents either a class template or a template template parameter.
* `I` represents an expression that either is an `I`, is value-dependent in `P` or `A`, or has the same constant value in `P` and `A`.
rrev|since=c++17|
* `noexcept(``I``)` represents an `exception specification` in which the possibly-implicit `noexcept` specifier's operand satisfies the rules for an `I` above.
If **`P`** has one of the forms that include a template parameter list **`<T>`** or **`<<nowiki/>I>`**, then each element **`P`**i of that template argument list is matched against the corresponding template argument **`A`**i of its **`A`**. If the last **`P`**i is a pack expansion, then its pattern is compared against each remaining argument in the template argument list of **`A`**. A trailing parameter pack that is not otherwise deduced, is deduced to an empty parameter pack.
...
rrev multi
|rev1=
* `X<int>(char[6])` is an example of `T` `(``U``)`, where `T` is `X<int>` and `U` is `char[6]`;
|since2=c++17|rev2=
* `X<int>(char[6])` is an example of `T` `(``U``)` `noexcept(``I``)`, where `T` is `X<int>`, `U` is `char[6]`, and `I` in the implicit `noexcept` specifier is `false`;
* `X<int>` is an example of `TT``&lt;T>`, where `TT` is `X` and `T` is `int`, and
* `char[6]` is an example of `T` `[``I``]`, where `T` is `char` and `I` is `std::size_t(6)`.
rrev multi|
|until1=c++17|rev1=
...
//   std::basic_ostream<char, std::char_traits<char>>&
// )
// (other overloads of operator<< are not viable) <!-- As is mentioned in {{rlp|overloaded address|address of a overload set
```
...
`Alias templates` are not deduced<sup>(since C++20)</sup> , except in `class template argument deduction`:
...
| cwg-322 | C++98 | type parameters of reference types were not<br>adjusted to use the referenced type for deduction | adjustment added |
| cwg-1387 | C++11 | the expression of a decltype-specifier was not a non-deduced context | it is |
| cwg-1391 | C++98 | effect of implicit conversions of the arguments<br>that aren't involved in deduction were not specified | specified as described above |
| cwg-1591 | C++11 | cannot deduce array bound and element type from a ''braced-init-list'' | deduction allowed |
| cwg-2052 | C++98 | deducing an operator with non-class<br>non-enum arguments was a hard error | soft error if there<br>are other overloads |
| cwg-2091 | C++98 | deducing a reference non-type parameter did not<br>work due to type mismatch against the argument | type mismatch avoided |

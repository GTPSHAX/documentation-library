...
# Transactional memory mark since tm_ts
...
The exceptions used for transaction cancellation in `atomic_cancel` blocks are `std::bad_alloc`, `std::bad_array_new_length`, `std::bad_cast`, `std::bad_typeid`, `std::bad_exception`, `std::exception` and all standard library exceptions derived from it, and the special exception type `cpp/error/tx_exception|std::tx_exception<T>`.
...
`cpp/keyword/atomic_cancel`,
`cpp/keyword/atomic_commit`,
`cpp/keyword/atomic_noexcept`,
`cpp/keyword/synchronized`,
`cpp/keyword/transaction_safe`,
`cpp/keyword/transaction_safe_dynamic`

## Compiler support

This technical specification is supported by GCC as of version 6.1 (requires `-fgnu-tm` to enable). An older variant of this specification was [http://www-users.cs.umn.edu/~boutcher/stm/ supported in GCC] as of 4.7.

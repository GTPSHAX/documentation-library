...
The `typedef` specifier may not appear in the declaration of a function parameter nor in the *decl-specifier-seq* of a :
...
For example, in }, `S` is a typedef name for linkage purposes. The class or enumeration type defined in this way has `external linkage` (unless it is in an unnamed namespace).
rrev|since=c++20|

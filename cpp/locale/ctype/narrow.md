...
Narrowing is always successful and is always reversible (by calling `widen()`) for all characters from the <sup>(until C++23)</sup> basic source character set<sup>(since C++23)</sup> basic character set.
* i.e. `1=do_widen(do_narrow(c, 0)) == c` always holds for any character `c` in the <sup>(until C++23)</sup> basic source character set<sup>(since C++23)</sup> basic character set.
Narrowing, if successful, preserves all character classification categories known to `is()`.
* i.e. `is(m, c)  is always `true` for any named `ctype` category with a `ctype<char>` facet `ctc` and `ctype_base::mask` value `m` (unless `do_narrow` returns `dflt`).

...
2. Returns an `std::string` holding, in each `char` element, the number of digits in each group of the numeric output formatted by `cpp/locale/num_put/put|num_put::put()` (and, therefore, `cpp/io/basic_ostream/operator_ltlt|basic_ostream::operator<<`).

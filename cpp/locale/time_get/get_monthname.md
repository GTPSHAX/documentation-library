---
title: std::time_get::get_monthname
type: Localizations
...
2. Reads successive characters from the sequence [beg, end) and parses out the month name (possibly abbreviated), using the default format for month names expected by this locale, which is the same format as `"%b"` as used by the functions `std::get_time`, , and the POSIX function `strptime()`.

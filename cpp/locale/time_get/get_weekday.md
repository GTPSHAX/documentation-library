---
title: std::time_get::get_weekday
type: Localizations
...
2. Reads successive characters from the sequence [beg, end) and parses out the weekday name (possibly abbreviated), using the default format for weekdays expected by this locale, which is the same format as `"%a"` as used by the functions `std::get_time`, , and the POSIX function `strptime()`.

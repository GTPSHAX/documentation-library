...
For two-digit input values, many implementations use the same parsing rules as the conversion specifier `'%y'` as used by `std::get_time`, `std::time_get::get()`, and the POSIX function `strptime()`: two-digit integer is expected, the values in the range  results in values 1969 to 1999, range  results in 2000 to 2068. Four-digit inputs are typically accepted as-is.

...
The overloads not taking `opts` as a parameter uses a default constructed instance of `cpp/memory/pool_options` as the options. The overloads not taking `upstream` as a parameter uses the return value of `std::pmr::get_default_resource` as the upstream memory resource.

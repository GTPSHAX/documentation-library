...
When enabled, for a given `1=std::unique_ptr<T, D> p`, this specialization ensures that
`1=std::hash<std::unique_ptr<T, D>>()(p) == std::hash<typename std::unique_ptr<T, D>::pointer>()(p.get())`.

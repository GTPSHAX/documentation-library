...
1. . Returns `std::tuple` determined as follows:
...
* Otherwise, if `std::uses_allocator_v<T, Alloc>` is `true` and `std::is_constructible_v<T, std::allocator_arg_t, const Alloc&, Args...>` is `true`, returns<br>c multi
...
2. . For `T` that is `std::pair<T1, T2>`, equivalent to
...
9. cpp/enable_if|`T` is a specialization of `std::pair`, and given the exposition-only function template
...
The overloads  provide allocator propagation into `std::pair`, which supports neither leading-allocator nor trailing-allocator calling conventions (unlike, e.g. `std::tuple`, which uses leading-allocator convention).

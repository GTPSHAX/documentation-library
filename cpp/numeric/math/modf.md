...
@1-3@ Decomposes given floating point value `num` into integral and fractional parts, each having the same type and sign as `num`. The integral part (in floating-point format) is stored in the object pointed to by `iptr`.<sup>(since C++23)</sup>   The library provides overloads of `std::modf` for all cv-unqualified floating-point types as the type of the parameter `num` and the pointed-to type of `iptr`.
rrev|since=c++11|

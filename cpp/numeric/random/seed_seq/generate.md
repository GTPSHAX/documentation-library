...
Generate unbiased seeds by filling the output range [begin, end) with 32-bit unsigned integer values, based on the (possibly biased) seeds stored in .
* If `1=begin == end` is `true`, does nothing.
...
If `RandomIt` does not meet the requirements of *RandomAccessIterator*, or it is not mutable, the behavior is undefined.
...
| dsc|}|built-in bitwise XOR | |
| dsc|}|built-in bitwise right-shift | |
| dsc||mathjax-or|\(\scriptsize x\ \mathsf{xor}\ (x\ \mathsf{rshift}\ 27) \)|x xor (x rshift 27) | |

The generation algorithm consists of the following steps, where  denotes `begin[i % n]`,  denotes :
...
:@1@ Let  be mathjax-or|\(\scriptsize 1664525 \cdot T(S_k\ \mathsf{xor}\ S_{k+p}\ \mathsf{xor}\ S_{k-1}) \)|1664525·T(S xor S xor S).
:@2@ Let  be +j, where  is:
* , if
* }, if
* , if
:@3@ Set } to mathjax-or|\(\scriptsize (S_{k+p}+r_1) \mod 2^{32} \)|(S+r) mod 2.
:@4@ Set } to mathjax-or|\(\scriptsize (S_{k+q}+r_2) \mod 2^{32} \)|(S+r) mod 2.
:@5@ Set  to }.
3. For each integer `k` in [m, m + n), performs the following operations in order:
:@1@ Let  be mathjax-or|\(\scriptsize 1566083941 \cdot T(S_k+S_{k+p}+S_{k-1}) \)|1566083941·T(S+S+S).
:@2@ Let  be -(k mod n).
:@3@ Set } to mathjax-or|\(\scriptsize (S_{k+p}\ \mathsf{xor}\ r_3) \mod 2^{32} \)|(S xor r) mod 2.
:@4@ Set } to mathjax-or|\(\scriptsize (S_{k+q}\ \mathsf{xor}\ r_4) \mod 2^{32} \)|(S xor r) mod 2.
:@5@ Set  to }.

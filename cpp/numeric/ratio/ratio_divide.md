...
The result is a `std::ratio` specialization `std::ratio<U, V>`, such that given `1=Num == R1::num * R2::den` and `1=Denom == R1::den * R2::num` (computed without arithmetic overflow), `U` is `std::ratio<Num, Denom>::num` and `V` is `std::ratio<Num, Denom>::den`.

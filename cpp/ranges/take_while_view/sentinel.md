---
title: std::ranges::take_while_view::sentinel
type: Ranges
source: https://en.cppreference.com/w/cpp/ranges/take_while_view/sentinel
---

ddcl|since=c++20|notes=|
template< bool Const >
class /*sentinel*/;
The return type of `take_while_view::end`.
The type `/*sentinel*/<true>` is returned by the const-qualified overload. The type `/*sentinel*/<false>` is returned by the non-const-qualified overload.

## Member types


| Item | Description |
|------|-------------|
| **Member** | Definition |


## Data members


| Item | Description |
|------|-------------|
| **Member** | Description |


## Member functions


| cpp/ranges/adaptor/sentinel/dsc constructor|take_while_view | (see dedicated page) |
| cpp/ranges/adaptor/sentinel/dsc base|take_while_view | (see dedicated page) |


## Non-member functions


| cpp/ranges/adaptor/sentinel/dsc operator cmp|take_while_view | (see dedicated page) |


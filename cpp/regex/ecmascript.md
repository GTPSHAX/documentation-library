...
The `ECMAScript` 3 regular expression grammar in C++ is [https://ecma-international.org/ecma-262/5.1/#sec-15.10 ECMA-262 grammar] with modifications marked with  below.
...
The values of the individual ''DecimalDigits'' are obtained by calling `std::regex_traits::value` on each of the digits.
...
1. The position that immediately follows a ''LineTerminator'' character <sup>(until C++17)</sup> (this may not be supported)<sup>(since C++17)</sup>  (this is only guaranteed if `cpp/regex/syntax_option_type|std::regex_constants::multiline` is enabled)
2. The beginning of the input (unless `std::regex_constants::match_not_bol` is enabled)
The assertion **`$`** (end of line) matches
1. The position of a ''LineTerminator'' character <sup>(until C++17)</sup> (this may not be supported)<sup>(since C++17)</sup> (this is only guaranteed if `cpp/regex/syntax_option_type|std::regex_constants::multiline` is enabled)
2. The end of the input (unless `std::regex_constants::match_not_eol` is enabled)
...
3. The beginning of input if the first character is a letter, digit, or underscore (unless `std::regex_constants::match_not_bow` is enabled)
4. The end of input if the last character is a letter, digit, or underscore (unless `std::regex_constants::match_not_eow` is enabled)
...
1. If `std::regex_constants::icase` is set, the characters are equal if the return values of `std::regex_traits::translate_nocase` are equal .
2. Otherwise, if `std::regex_constants::collate` is set, the characters are equal if the return values of `std::regex_traits::translate` are equal .
3. Otherwise, the characters are equal if `1=operator==` returns `true`.
...
''ControlLetter'' is any lowercase or uppercase ASCII letters and this character escape matches the character whose code unit equals the remainder of dividing the value of the code unit of ''ControlLetter'' by `32`. For example, **`\cD`** and **`\cd`** both match code unit `U+0004` (EOT) because 'D' is `U+0044` and `1= 0x44 % 32 == 4`, and 'd' is `U+0064` and `1= 0x64 % 32 == 4`.
...
''UnicodeEscapeSequence'' is the letter **`u`** followed by exactly four ''HexDigit''s. This character escape matches the character whose code unit equals the numeric value of this four-digit hexadecimal number. If the value does not fit in this `std::basic_regex`'s `CharT`, `std::regex_error` is thrown .
...
| - |
| CharacterClassEscape | ClassName expressionmark | C++ only | Meaning |
| - |
| ttb | d |
...
If non-empty class range has the form **`''ClassAtom'' - ''ClassAtom''`**, it matches any character from a range defined as follows:
...
1. If `std::regex_constants::collate` is not on, the character is matched by direct comparison of code points: `c` is matched if `c1 <
...
2. The resulting strings are compared using `std::regex_traits::transform` and the character `c` is matched if `transformed c1 <
...
When using `std::regex_constants::collate`, collating elements can always be used as ends points of a range (e.g. `<nowiki>[[</nowiki>.dz.]-g]` in Hungarian).
ClassAtomEquivalence ::
: **`[=`** ''ClassName'' **`=]`**
...
A primary sort key is one that ignores case, accentuation, or locale-specific tailorings; so for example `<nowiki>=a=</nowiki>` matches any of the characters: `a, À, Á, Â, Ã, Ä, Å, A, à, á, â, ã, ä and å. `
ClassName ::
: ClassNameCharacter

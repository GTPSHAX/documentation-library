---
title: std::regex_traits::translate_nocase
...
When the regex library needs to match two characters `c1` and `c2` and the flag `std::regex_constants::icase` is `true`, it executes `1=regex_traits<>::translate_nocase(c1) == regex_traits<>::translate_nocase(c2)`.

...
::@a@ end-of-file condition on `input`, in which case, `getline` sets `cpp/io/ios_base/iostate|eofbit`.
...
::@c@ `str.max_size()` characters have been stored, in which case `getline` sets `cpp/io/ios_base/iostate|failbit` and returns.
:@3@ If no characters were extracted for whatever reason (not even the discarded delimiter), `getline` sets `cpp/io/ios_base/iostate|failbit` and returns.

...
| cpp/thread/lock_guard/dsc destructor | (see dedicated page) |
| cpp/thread/lock_guard/dsc operator{{= | (see dedicated page) |


## Notes

A common beginner error is to "forget" to give a `lock_guard` variable a name, e.g. `std::lock_guard(mtx);` (which default constructs a `lock_guard` variable named `mtx`) or } (which constructs a prvalue object that is immediately destroyed), thereby not actually constructing a lock that holds a mutex for the rest of the scope.
rrev|since=c++17|
`cpp/thread/scoped_lock|std::scoped_lock` offers an alternative for `lock_guard` that provides the ability to lock multiple mutexes using a deadlock avoidance algorithm.

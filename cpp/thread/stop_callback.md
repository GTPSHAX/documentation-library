...
class stop_callback;
The `stop_callback` class template provides an RAII object type that registers a callback function for an associated `cpp/thread/stop_token|std::stop_token` object, such that the callback function will be invoked when the `cpp/thread/stop_token|std::stop_token`'s associated `cpp/thread/stop_source|std::stop_source` is requested to stop.
Callback functions registered via `stop_callback`'s constructor are invoked either in the same thread that successfully invokes `request_stop()` for a `cpp/thread/stop_source|std::stop_source` of the `stop_callback`'s associated `cpp/thread/stop_token|std::stop_token`; or if stop has already been requested prior to the constructor's registration, then the callback is invoked in the thread constructing the `stop_callback`.
More than one `stop_callback` can be created for the same `cpp/thread/stop_token|std::stop_token`, from the same or different threads concurrently. No guarantee is provided for the order in which they will be executed, but they will be invoked synchronously; except for `stop_callback`(s) constructed after stop has already been requested for the `cpp/thread/stop_token|std::stop_token`, as described previously.
...
The template param `Callback` type must be both  and . Any return value is ignored.
...
| cpp/thread/stop_callback/dsc constructor | (see dedicated page) |
| cpp/thread/stop_callback/dsc destructor | (see dedicated page) |
| cpp/thread/stop_callback/dsc operator{{= | (see dedicated page) |

...
If the `stop_token` object has associated stop-state from a `std::jthread`&mdash;for example, the `stop_token` was retrieved by invoking `get_stop_token()` on a `std::jthread` object&mdash;then this function always returns `true`. A `std::jthread` always has an internal `std::stop_source` object, even if the thread's invoking function does not check it.

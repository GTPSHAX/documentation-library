---
title: std::numeric_limits::max_exponent
type: Utilities
...
The value of `std::numeric_limits<T>::max_exponent` is the largest positive number `n` such that }, where `r` is `std::numeric_limits<T>::radix`, is a representable finite value of the floating-point type `T`.
